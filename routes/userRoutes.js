...
router.post('/help', async (req, res) => {
  try {
    const {selectIssue, username, phonenumber, selectYouAre, query} = req.body;
    const newUser = new Help({selectIssue, username, phonenumber, selectYouAre, query});
    await newUser.save();
    res.status(201).send('help recorded successfully');
  } catch (error) {
    res.status(400).send({ message: error.message });
  };
});
